// client.js — 시타델 클라이언트 (멀티플레이어 완성판)
'use strict';
...
let G  = null;
let MY_ID   = null;
let MY_ROOM = null;   // 'LOCAL' or room code
let IS_HOST = false;
let AI_SOLO = 3;
let AI_WAIT = 0;
...
// UI 상태 (항상 로컬에서만 관리)
let selCard     = null;
let pendAbility = null;   // 'assassin'|'thief'|'warlord'|'wizard'
let wizMode     = null;   // null|'swap'|'discard'
let wizDiscSel  = [];
let warlordTpi  = null;
...
function notify(msg, type = 'info') {
  ...
  ntTimer = setTimeout(() => e.className = '', 3000);
}
...
function connectWS(onReady) {
  ...
  ws.onopen  = () => { setConn('ok','✅ 서버 연결됨'); if (onReady) onReady(); };
  ws.onclose = () => setConn('err','❌ 연결 끊김 — 새로고침하세요');
  ws.onerror = () => { setConn('err','❌ 연결 오류'); notify('서버 연결 실패. 서버가 실행 중인지 확인하세요.','bad'); };
...
function wsSend(obj) {
  if (!ws || ws.readyState !== 1) { console.warn('[WS] 전송실패:', obj.type); return; }
  ws.send(JSON.stringify(obj));
}

function setConn(t, x) {
  const e = $('connSt'); if (!e) return;
  e.className = `conn-${t}`; e.textContent = x;
}

// 호스트: G 변경 후 모든 비호스트에게 전송
function syncState() {
  if (MY_ROOM === 'LOCAL' || !IS_HOST || !G) return;
  wsSend({ type: 'sync_state', G, gameOver: !!G.gameOver });
}

// 비호스트: 행동을 호스트에게 전송만 (로컬 실행 안 함)
// 결과는 호스트의 state_update로 받음
...
$('tabs').addEventListener('click', e => {
  ...
  if (t.dataset.t === 'join') {
    if (!ws || ws.readyState !== 1) connectWS(() => wsSend({type:'get_room_list'}));
    else wsSend({type:'get_room_list'});
  }
});
...
function renderRoomList(rooms) {
  ...
  rooms.forEach(r => {
    ...
    const avatars = (r.players || []).map(p => `<span title="${esc(p.name)}">${p.avatar}</span>`).join('');
    const d = mkEl('div', 'rl-item');
    d.innerHTML = `
      <div class="rl-info">
        <div class="rl-name">🏠 ${esc(r.title)}</div>
        <div class="rl-meta">
          <span class="dot"></span>
          방장: <b>${esc(r.hostName)}</b>
          &nbsp;
          <span class="rl-avatars">${avatars}</span>
...
function quickJoin(code) {
  ...
  goWaitScreen(false);
  const doJoin = () => wsSend({ type:'join_room', playerId:MY_ID, name, code });
  if (!ws || ws.readyState !== 1) connectWS(doJoin); else doJoin();
}
...
function doCreateRoom() {
  ...
  goWaitScreen(true);
  const doCreate = () => wsSend({ type:'create_room', playerId:MY_ID, name, title:`${name}의 방` });
  if (!ws || ws.readyState !== 1) connectWS(doCreate); else doCreate();
}
...
function renderWaitRoom(room) {
  ...
  (room.players || []).forEach((p, i) => {
    ...
    const d = mkEl('div', 'w-player' + (i === 0 ? ' host' : ''));
    d.innerHTML = `
      <span class="dot"></span>
...
function onGameStart(room) {
  if (IS_HOST) {
    ...
    enterGame();
    aiAutoAll();
    syncState();  // 비호스트에게 초기 상태 즉시 전송
    render();
  } else {
    enterGame();
    // 비호스트: G는 state_update로 수신, 그 전까지 로딩 표시
    const m = $('mainArea');
...
function goLobby() {
  ...
  if (ws && ws.readyState === 1) wsSend({type:'get_room_list'});
}
...
// 호스트 전용: 원격 행동 처리
// ══════════════════════════════════════════════
function applyRemote(playerId, action) {
...
// ══════════════════════════════════════════════
function aiAutoAll() {
  // 호스트(또는 솔로): AI 턴을 자동 건너뜀. 실제 플레이어 차례에서 멈춤.
  while (G.phase === 'select_character' && G.selIdx < G.players.length) {
    const pi = G.selOrder[G.selIdx];
    if (pi === myIdx()) break;                              // 내 차례
    if (!G.players[pi].isAI && MY_ROOM !== 'LOCAL') break; // 다른 실제 플레이어
    // AI 자동 선택
...
function selectCharacter(charId) {
  ...
  if (MY_ROOM !== 'LOCAL' && !IS_HOST) {
    // 비호스트: 서버에 전송만. 결과는 state_update로 받음.
...
function advChar(idx) {
  ...
  if (G.assassinTarget === idx) {
    ...
    if (ki !== undefined) {
      feed('💀', `<b>${G.players[ki].name}</b>이(가) 암살당해 스킵!`, 'combat');
      advChar(idx + 1); return;
    }
  }
  ...
  if (G.thiefTarget === idx && G.thiefPi !== null && G.thiefPi !== pi) {
    ...
    feed('🦹', `<b>${G.players[G.thiefPi].name}(도둑)</b>이(가) 💰${stolen} 탈취!`, 'combat');
  }
...
function execTakeGold(pi) {
  ...
  if (p.selectedCharacter?.id === 7) {
    ...
    feed('🏗️',`<b>${p.name}(건축가)</b> 카드 2장 추가`,'ability');
  }
  G.actionPhase = 'build';
  syncState(); render();
...
function execDrawCard(pi) {
  ...
  if (hasObs) {
    G._obsCards = [deckPop(),deckPop(),deckPop()].filter(Boolean);
    G.actionPhase = 'observatory';
    syncState(); render(); return;
  }
  if (hasLib) {
    const c1=deckPop(),c2=deckPop();
    if(c1)p.hand.push(c1); if(c2)p.hand.push(c2);
    feed('📚',`<b>${p.name}(도서관)</b> 카드 2장 획득`,'ability');
  } else {
    G._drawOpts = [deckPop(),deckPop()].filter(Boolean);
...
function chooseDraw(uid_) {
  if (MY_ROOM !== 'LOCAL' && !IS_HOST) { sendAction({type:'choose_draw',uid:uid_}); return; }
  execChooseDraw(myIdx(), uid_);
}
function execChooseDraw(pi, uid_) {
  const opts = G._drawOpts || G._obsCards || [];
  const chosen = opts.find(c => c.uid === uid_); if (!chosen) return;
  G.players[pi].hand.push(chosen);
  G.discard.push(...opts.filter(c => c.uid !== uid_));
...
function execBuild(pi, uid_) {
  ...
  p.gold-=card.cost; p.hand=p.hand.filter(c=>c.uid!==uid_); p.city.push(card); p.buildsLeft--;
...
function useAbility() {
  if (!isMyTurn()) return;
  if (abilityDone) { notify('이미 사용했습니다!','warn'); return; }
...
function cancelAbility() {
  pendAbility=null;wizMode=null;wizDiscSel=[];warlordTpi=null;selCard=null;render();
}
...
function buildAISteps(pi) {
  ...
  s.push({t:'ann',pi});
  if(ch?.id===1&&G.assassinTarget===null){const oth=Object.entries(G.selectedChars).filter(([k,v])=>+k!==1&&+v!==pi);if(oth.length){const t=oth[Math.floor(Math.random()*oth.length)];s.push({t:'assassin',pi,cid:+t[0]});}}
  if(ch?.id===2&&G.thiefTarget===null){const oth=Object.entries(G.selectedChars).filter(([k,v])=>+k>2&&+v!==pi&&G.assassinTarget!==+k);if(oth.length){const t=oth[Math.floor(Math.random()*oth.length)];s.push({t:'thief',pi,cid:+t[0]});}}
  ...
  s.push({t:'income',pi});
  if(ch?.id===8&&Math.random()>.5){const en=G.players.filter((ep,ei)=>ei!==pi&&ep.city.length>0&&ep.city.length<7&&!(ep.selectedCharacter?.id===5&&G.assassinTarget!==5));if(en.length){const enemy=en[Math.floor(Math.random()*en.length)];const ei=G.players.indexOf(enemy);const cv=enemy.city.reduce((a,b)=>a.cost<b.cost?a:b);const cost=Math.max(0,cv.cost-1);if(p.gold>=cost+2)s.push({t:'warlord',pi,ei,uid:cv.uid});}}
...
function applyAIStep(s) {
  ...
  switch(s.t){
    case 'ann': feed(ch?.icon||'⏳',`<b>${p.name}</b>(${ch?.name||'?'}) 턴 시작.`,'system'); break;
    case 'assassin': G.assassinTarget=s.cid;feed('🗡️',`<b>${p.name}(암살자)</b> <b>${CHARS.find(c=>c.id===s.cid)?.name}</b> 암살!`,'combat');break;
    ...
    case 'draw': {const c=deckPop();if(c){p.hand.push(c);feed('🃏',`<b>${p.name}</b> 카드 뽑기`,'card');}break;}
    ...
    case 'warlord': {const tp=G.players[s.ei],d=tp?.city.find(c=>c.uid===s.uid);if(d&&tp){const cost=Math.max(0,d.cost-1);if(p.gold>=cost){p.gold-=cost;tp.city=tp.city.filter(c=>c.uid!==s.uid);G.discard.push(d);feed('⚔️',`<b>${p.name}(장군)</b> ${d.icon}<b>${d.name}</b> 파괴! (💰-${cost})`,'combat');}}break;}
    case 'build': {const d=p.hand.find(c=>c.uid===s.uid);if(d&&p.gold>=d.cost&&!p.city.find(b=>b.id===d.id)){p.gold-=d.cost;p.hand=p.hand.filter(c=>c.uid!==s.uid);p.city.push(d);feed('🏛️',`<b>${p.name}</b> ${d.icon}<b>${d.name}</b> 건설! (💰${p.gold})`,'build');if(p.city.length>=7){if(!G.players.some(x=>x.complete)){p.firstComplete=true;p.complete=true;feed('🎉',`<b>${p.name}</b> 도시 완성!`,'win');}else p.complete=true;}}break;}
...
function showGameOver() {
  ...
  ['🥇','🥈','🥉','4️⃣','5️⃣','6️⃣'].forEach((m,i)=>{
    ...
    const li=mkEl('li','go-sc');
    li.innerHTML=`<span>${m} ${p.avatar} ${esc(p.name)}</span><span class="go-pts">${p.score}점</span>`;
    list.appendChild(li);
  });
...
function rTurnBar(){
  ...
  const lbl=mkEl('div','to-lbl');lbl.textContent=G.phase==='select_character'?'선택 순서':'행동 순서';bar.appendChild(lbl);
  if(G.phase==='select_character'){
    G.selOrder.forEach((pi,i)=>{
      const p=G.players[pi],done=i<G.selIdx,cur=i===G.selIdx;
      const d=mkEl('div','to-item'+(done?' done':'')+(cur?' cur':''));
      ...
      bar.appendChild(d);
      if(i<G.selOrder.length-1){const a=mkEl('div','to-arr');a.textContent='→';bar.appendChild(a);}
    });
  }else{
    for(let cid=1;cid<=8;cid++){
      ...
      const d=mkEl('div','to-item'+(done?' done':'')+(cur?' cur':'')+(dead?' dead':''));
      ...
      bar.appendChild(d);
      if(Object.keys(G.selectedChars).some(k=>+k>cid)){const a=mkEl('div','to-arr');a.textContent='→';bar.appendChild(a);}
    }
  }
}

function rPlayerList(){
  const cont=$('pList');cont.innerHTML='';
  const mi=myIdx();
  G.players.forEach((p,i)=>{
    ...
    let cls='pcard'+(isMe?' me':'')+(isTurn?' active':'')+(isDead?' dead':'')+(isWarlord||isWizSwap?' tsel':'');
    const d=mkEl('div',cls);
    if(isWarlord)d.onclick=()=>{warlordTpi=i;renderWarlordOverlay(i);};
    if(isWizSwap)d.onclick=()=>doWizardSwap(i);
...
function renderFeed(){
  ...
  G.log.slice(0,35).forEach(e=>{
    const d=mkEl('div',`fe ev-${e.type||'system'}`);
...
function rCharSelect(main){
  ...
  const ban=mkEl('div','banner bn-sel');
  ban.innerHTML=`<div class="bn-ico">🎭</div><div><div class="bn-title" style="color:#c39bd3">${myTurn?'⚡ 캐릭터를 선택하세요!':'⏳ 선택 대기 중...'}</div><div class="bn-desc">${myTurn?`<b>${G.availChars.length}개</b> 중 하나를 선택하세요`:me.selectedCharacter?`<b>${me.selectedCharacter.icon}${me.selectedCharacter.name}</b> 선택 완료 — 다른 플레이어 대기 중...`:'다른 플레이어가 선택 중입니다.'}</div></div>`;
  main.appendChild(ban);
  if(!myTurn&&me.selectedCharacter){
    const s=mkEl('div','my-strip');const ch=me.selectedCharacter;
  ...
  if(!myTurn)return;
  const t=mkEl('div','sect');t.textContent='선택 가능한 캐릭터';main.appendChild(t);
  const grid=mkEl('div','csel-grid');
  G.availChars.forEach(ch=>{
    const c=mkEl('div','csel-card');
...
function rTurnPhase(main){
  ...
  // 비호스트는 자신의 차례도 행동 버튼은 있지만 결과를 기다려야 함을 표시
  const waitingForServer = !IS_HOST&&MY_ROOM!=='LOCAL'&&G.curPi===mi&&G.actionPhase!=='choose';

  const ban=mkEl('div',`banner ${myTurn?'bn-act':'bn-wait'}`);
  ban.innerHTML=`<div class="bn-ico">${myTurn?'⚡':curP?.selectedCharacter?.icon||'⏳'}</div><div><div class="bn-title" style="color:${myTurn?'var(--gold2)':'var(--text)'}">${myTurn?'⚡ 내 턴!':aiRunning?`${curP?.name} 행동 중...`:`${curP?.name}의 턴`}</div><div class="bn-desc">${myTurn?G.actionPhase==='choose'?'💰 금화 받기 또는 🃏 카드 뽑기 선택':isDC?'카드 1장을 선택하세요 (나머지 버림)':isObs?'3장 중 1장 선택 (천문대)':pendAbility?'타깃 선택 또는 취소':'🏛️ 건설하거나 능력 사용 후 턴 종료':'기다리는 중...'}</div></div>`;
  main.appendChild(ban);

  if(ch){
    const s=mkEl('div','my-strip');s.style.cssText=`background:${ch.bg};border-color:${ch.bc};border-radius:9px;`;
  ...
  if(pendAbility){
    const msgs={assassin:'🗡️ 우측 캐릭터 목록에서 암살할 캐릭터를 클릭하세요',thief:'🦹 우측 캐릭터 목록에서 훔칠 캐릭터를 클릭하세요 (1·2번 제외)',warlord:'⚔️ 좌측 플레이어를 클릭해 파괴할 건물을 선택하세요',wizard:'🔮 아래에서 마술사 능력을 선택하세요'};
    const h=mkEl('div','hint');h.innerHTML=msgs[pendAbility]||'';main.appendChild(h);
  }

  if(pendAbility==='wizard'&&wizMode===null){
    const wp=mkEl('div','wiz-panel');
    wp.innerHTML=`<div class="wiz-title">🔮 마술사 능력 선택</div>`;
    const b1=mkEl('button','wiz-btn');b1.innerHTML='① 다른 플레이어와 손패 전체 교환<br><small style="color:var(--dim)">👈 좌측에서 플레이어 클릭</small>';b1.onclick=()=>{wizMode='swap';rPlayerList();render();};
    const b2=mkEl('button','wiz-btn');b2.innerHTML='② 손패 일부 버리고 새로 뽑기<br><small style="color:var(--dim)">아래 손패에서 버릴 카드 선택</small>';b2.onclick=()=>{wizMode='discard';wizDiscSel=[];render();};
  ...
  if((isDC&&G._drawOpts)||(isObs&&G._obsCards)){
    ...
    const t=mkEl('div','sect');t.textContent=isObs?'🔭 천문대: 3장 중 1장 선택':'🃏 카드 선택 (1장 고르기)';main.appendChild(t);
  ...
  if(myTurn&&G.actionPhase==='choose'){
    const t=mkEl('div','sect');t.textContent='수입 선택';main.appendChild(t);
    const row=mkEl('div','inc-row');
    const g=mkEl('div','inc-btn inc-g');g.innerHTML=`<div class="inc-ico">💰</div><div class="inc-lbl">금화 2개 받기</div><div class="inc-sub">안정적 수입</div>`;g.onclick=takeGold;
    const c=mkEl('div','inc-btn inc-c');c.innerHTML=`<div class="inc-ico">🃏</div><div class="inc-lbl">카드 뽑기</div><div class="inc-sub">2장 보고 1장 선택 (덱: ${G.deck.length}장)</div>`;c.onclick=drawCard;
    row.appendChild(g);row.appendChild(c);main.appendChild(row);
  }

  const ht=mkEl('div','sect');ht.textContent=`🃏 내 손패 (${me.hand.length}장)`;main.appendChild(ht);
  const isWD=myTurn&&pendAbility==='wizard'&&wizMode==='discard';
  if(!me.hand.length){const e=mkEl('div','empty');e.textContent='손패 없음';main.appendChild(e);}
  else{
    const grid=mkEl('div','hand-grid');
    ...
    if(isWD&&wizDiscSel.length>0){
      const b=mkEl('button','ab ab-purple');b.style.marginTop='7px';
      b.textContent=`🔮 ${wizDiscSel.length}장 버리고 새로 뽑기`;
      b.onclick=()=>{
        const a={type:'ability',abilityType:'wizard_discard',uids:wizDiscSel};
        if(MY_ROOM!=='LOCAL'&&!IS_HOST)sendAction(a);else execAbility(mi,a);
      };
      main.appendChild(b);
    }
  }

  const ct=mkEl('div','sect');ct.textContent=`🏛️ 내 도시 (${me.city.length}/7) — ⭐${calcScore(me)}점`;main.appendChild(ct);
  if(!me.city.length){const e=mkEl('div','empty');e.textContent='건물 없음';main.appendChild(e);}
  else{
    const grid=mkEl('div','hand-grid');
    me.city.forEach(card=>{const c=mkCard(card);c.classList.add('disabled');c.style.pointerEvents='auto';c.style.cursor='pointer';c.onclick=()=>openTT(card);grid.appendChild(c);});
    main.appendChild(grid);
  }
...
function renderWarlordOverlay(tpi){
  const main=$('mainArea');
  const old=document.getElementById('woOv');if(old)old.remove();
  const tp=G.players[tpi];if(!tp)return;
  const wrap=mkEl('div','wo-wrap');wrap.id='woOv';
  wrap.innerHTML=`<div class="wo-title">⚔️ ${esc(tp.name)}의 건물 — 파괴할 건물 선택</div>`;
  const grid=mkEl('div','wo-grid');
  ...
  tp.city.forEach(dist=>{
    ...
    if(!canAfford){c.classList.add('disabled');c.title=`💰${cost} 필요`;}
      ...
      c.onclick=()=>{
        const a={type:'ability',abilityType:'warlord',targetPi:tpi,distUid:dist.uid};
        if(MY_ROOM!=='LOCAL'&&!IS_HOST)sendAction(a);else execAbility(myIdx(),a);
      };
      const badge=mkEl('div','wo-cost');badge.textContent=cost===0?'무료':`💰${cost}`;c.appendChild(badge);
    }
    grid.appendChild(c);
  });
  wrap.appendChild(grid);
  const cancel=mkEl('button','ab ab-dim');cancel.style.marginTop='7px';cancel.textContent='✖️ 취소';cancel.onclick=cancelAbility;wrap.appendChild(cancel);
...
function rCharPanel(){
  ...
  CHARS.forEach(ch=>{
    ...
    const d=mkEl('div','ci'+(isRm?' removed':'')+(isMine?' mine':'')+(canT?' tmode':''));
...
function rActionBar(){
  const bar=$('abtns');bar.innerHTML='';
  if(!G)return;
  if(G.phase==='select_character'){
    if(!isMyCharSel()){const w=mkEl('div','ab-wait');w.innerHTML=`<span class="ab-dot"></span> 캐릭터 선택 대기 중...`;bar.appendChild(w);}
    return;
  }
  if(!isMyTurn()){
...
function abt(container,label,cls,fn){
  const b=mkEl('button',cls);b.innerHTML=label;
  if(fn)b.onclick=fn;else b.disabled=true;
  container.appendChild(b);return b;
}

function mkCard(d){
  const c=mkEl('div',`dcard c-${d.color}`);
